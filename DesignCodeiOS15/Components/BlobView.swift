...
struct BlobView: View {
  ...
  var body: some View {
    TimelineView(.animation) { timeline in
      ...
      let angle = Angle.degrees(now.remainder(dividingBy: 3) * 60) // every 3 secs, * 120 is to get 360deg, gives -180 to 180
    ...
    .onAppear {
      withAnimation(.linear(duration: 20).repeatForever(autoreverses: false).delay(1)) {
        appear = true
      }
    }
  ...
  func path(in rect: CGRect, x: Double, x2: Double) -> Path { // passing in x from above
    var path = Path()
